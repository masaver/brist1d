...
param_spaces = {
    'default': {
        'n_estimators': (100, 300),  # Common range for boosting rounds
        'learning_rate': (0.03, 0.1, 'log-uniform'),  # Standard learning rates
        'max_depth': (3, 7),  # Default depth range that balances depth and overfitting
        'min_child_weight': (1, 5),  # Moderate range to allow small variations in regularization
        'subsample': (0.7, 0.85),  # Light subsampling for diversity while preserving data
        'colsample_bytree': (0.7, 0.85),  # Common column sampling range
        'gamma': (0, 3),  # Small gamma range for light regularization
        'alpha': (1e-3, 0.1, 'log-uniform'),  # Default L1 regularization
        'lambda': (1e-3, 0.1, 'log-uniform'),  # Default L2 regularization
    },
    'deep': {
        'n_estimators': (50, 1000),  # Number of boosting rounds
        'learning_rate': (0.001, 0.3, 'log-uniform'),  # Step size shrinkage, log-uniform for finer control
        'max_depth': (3, 15),  # Maximum tree depth for base learners
        ...
        'colsample_bytree': (0.3, 1.0),  # Subsample ratio of columns when constructing each tree
        'gamma': (0, 10),  # Minimum loss reduction required to make a further partition on a leaf node
        'alpha': (1e-10, 10.0, 'log-uniform'),  # L1 regularization term on weights
        'lambda': (1e-10, 10.0, 'log-uniform'),  # L2 regularization term on weights
        'scale_pos_weight': (1, 100),  # Controls balance of positive and negative weights, often used for imbalanced classes
    },
    'fast': {
