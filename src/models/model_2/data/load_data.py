...
filename_timestamps = os.path.join(os.path.dirname(__file__), 'timestamps_ids.csv')


def load_data(recreate: bool = False) -> tuple[pd.DataFrame, pd.Series, pd.DataFrame]:
    if os.path.exists(filename_x_train) and os.path.exists(filename_y_train) and os.path.exists(filename_x_test) and os.path.exists( filename_timestamps):
        if not recreate:
            ...
            time_stamps_df = pd.read_csv(filename_timestamps, index_col=0)

            return X_train, y_train, X_test, time_stamps_df
    ...
    time_stamps_df.to_csv(filename_timestamps)
